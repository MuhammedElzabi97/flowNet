    ...
    <div className="w-screen sm:w-[calc(100vw-240px)] lg:max-w-2xl overflow-x-auto px-4">
              ...
              <div className="absolute inset-0 z-0 rounded-lg bg-black overflow-hidden">
                {story.media_type === "image" ? (
                  <img
                    src={story.media_url}
                    alt=""
                    className="h-full w-full object-cover hover:scale-110 transition duration-200 opacity-80"
                  />
                ) : (
                  <video
                    src={story.media_url}
      ...
      {showModal && (
        <StoryModal
          ...
          fetchStories={fetchStories}      // للمستقبل مع الـ API لو حبيت
          onCreateStory={handleAddStory}  // هذي اللي تهمنا الآن
        />
      )}
